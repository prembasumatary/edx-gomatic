...
def host_rest_client_repr(self):
    return """HostRestClient("{0}", config['gocd_username'], config['gocd_password'], ssl={1})""".format(self.__host, self.__ssl)


# patch the new formatting
go_cd_configurator.GoCdConfigurator.as_python = as_python
# does not work, repr method can't find __host or __ssl
