...
def install_pipeline(save_config_locally=False, dry_run=False, variable_files=[], cmd_line_vars={}):
    ...
    job.add_task(ExecTask(['/bin/bash', '-c', "touch github_key.pem && "
    ...
    job.add_task(ExecTask(['/bin/bash', '-c', "/usr/bin/git fetch && "
                                              "/usr/bin/git pull && "
    ...
    # run the edxapp play
    job.add_task(ExecTask(['/bin/bash', '-c', 'export ANSIBLE_HOST_KEY_CHECKING=False;'
